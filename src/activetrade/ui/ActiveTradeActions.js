...
module.exports = {
  activeTrade: (
    ...
    firebaseRef
      .database()
      .ref('/purchaserequests/' + _countryCode + '/' + purchaseRequestId)
  ...
  sellerConfirmsTrade: (
    ...
    try {
      let etherAmount = web3.toBigNumber(
        web3.toWei(Number(purchaseRequest.etherAmount), 'ether')
      )
      ...
      if (coinbase.toString() === purchaseRequest.sellerAddress) {
        ...
        event.watch(function(error, result) {
          ...
          if (result.args.uid === purchaseRequestId) {
            ...
            // START FIREBASE
            var now = new Date()
            var updatedPurchaseRequest = Object.assign({}, purchaseRequest, {
            ...
            firebaseRef
              .database()
              .ref(
                '/purchaserequests/' + seller.country + '/' + purchaseRequestId
              )
              .set(updatedPurchaseRequest, function(error) {
                ...
                dispatch(updateConfirmButtonIsDisabled(false))
                notificationHelpers.sendSellerConfirmsTradeNotification(
                  seller,
                  buyer,
                  purchaseRequest,
                  purchaseRequestId
        ...
        let value =
          purchaseRequest.advertiser === coinbase
            ? etherAmount.mul(1.01)
            : etherAmount
  ...
  sellerReleasesEther: (
    ...
    try {
      ...
      */
      ...
      var event = exchange.OrderCompleted({
        uid: purchaseRequestId,
        seller: coinbase,
        buyer: purchaseRequest.buyerAddress
      })
      event.watch(function(error, result) {
        if (result.args.uid === purchaseRequestId) {
          ...
          if (!error) {
            ...
            firebaseRef
              ...
              .set(updatedPurchaseRequest, function(error) {
                dispatch(sendEtherState('init'))
                event.stopWatching()
                notificationHelpers.sendSellerReleasesEtherNotification(
                  seller,
                  buyer,
                  purchaseRequest,
                  purchaseRequestId
      ...
      exchange.completeOrder(purchaseRequestId, { from: coinbase }, function(
        error,
        result
      ) {
        if (!error) {
  ...
  tradePostProcessing: (user, purchaseRequest, purchaseRequestId, users) => {
    ...
    if (!purchaseRequest.postProcessingCompleted) {
      var ref = firebaseRef
        .database()
        .ref('/disputes/' + purchaseRequestId)
      ...
      purchaseRequestHelpers.updateUserTradingStats(
        purchaseRequest,
      ...
      purchaseRequestHelpers.updateUserTradingStats(
        purchaseRequest,
        purchaseRequest.sellerUid,
        users
      )
      purchaseRequestHelpers.removePurchaseRequestFromActiveTrades(
        purchaseRequest.buyerUid,
        purchaseRequestId
      )
      purchaseRequestHelpers.removePurchaseRequestFromActiveTrades(
      ...
      purchaseRequestHelpers.addPurchaseRequestToCompletedTrades(
        purchaseRequest.buyerUid,
        purchaseRequestId,
  ...
  sellerRaisesDispute: (
    ...
    notificationHelpers.sendSellerRaisesDispute(
      seller,
      buyer,
      purchaseRequest,
      purchaseRequestId
  ...
  buyerRaisesDispute: (
    ...
    notificationHelpers.sendBuyerRaisesDispute(
      seller,
      buyer,
      purchaseRequest,
      purchaseRequestId
  ...
  arbiterReleasesToSeller: (
    ...
    try {
      ...
      if (!ethUtil.isValidAddress(process.env.DISPUTE_CONTRACT_ADDRESS)) {
            ...
            if (!error) {
              ...
              firebaseRef
                ...
                .set(updatedPurchaseRequest, function(error) {
                  dispatch(sendEtherState('init'))
                  event.stopWatching()
                  notificationHelpers.sendArbiterReleasesToSeller(
                    seller,
                    buyer,
                    purchaseRequest,
                    purchaseRequestId
        ...
        _instance.resolveDisputeSeller(
          purchaseRequestId,
          { from: coinbase },
          function(error, result) {
            if (!error) {
          ...
          if (result.args.uid === purchaseRequestId) {
            ...
            firebaseRef
              ...
              .set(updatedPurchaseRequest, function(error) {
                dispatch(sendEtherState('init'))
                event.stopWatching()
                notificationHelpers.sendArbiterReleasesToBuyer(
                  seller,
                  buyer,
                  purchaseRequest,
                  purchaseRequestId
  ...
  sellerRatesBuyer: (
    ...
    firebaseRef
      .database()
      .ref('/traderating/' + purchaseRequest.buyerUid + '/' + purchaseRequestId)
      .set({ rating: rating })
      .then(function() {
        //this needs to be moved when the trade feedback component unmounts
        firebaseRef
          .database()
          .ref('/traderating/' + purchaseRequest.buyerUid)
          .once('value', function(snap) {
            var totalRating = 0
            ...
            firebaseRef
              .database()
              .ref('/users/' + purchaseRequest.buyerUid + '/avgFeedback')
              .set(averageFeedback.toFixed(1))
          })
      })
  },
  buyerRatesSeller: (
    ...
    firebaseRef
      .database()
      .ref(
        '/traderating/' + purchaseRequest.sellerUid + '/' + purchaseRequestId
      )
      .set({ rating: rating })
      .then(function() {
        //this needs to be moved when the trade feedback component unmounts
        firebaseRef
          .database()
          .ref('/traderating/' + purchaseRequest.sellerUid)
          .once('value', function(snap) {
            var totalRating = 0
            var ratings = snap.val()
            console.log(ratings)
            ...
            firebaseRef
              .database()
              .ref('/users/' + purchaseRequest.sellerUid + '/avgFeedback')
              .set(averageFeedback.toFixed(1))
          })
      })
  },
