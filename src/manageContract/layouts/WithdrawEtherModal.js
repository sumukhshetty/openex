...
export default class WithdrawEther extends Component {
  render() {
    return (
      <div
        className="flex x absolute--fill fixed bg-black-80 z-1"
        ...
        <div className="flex col x">
          <div className="gradient w-100 pa4 shadow-1 flex col cxc">
            {this.props.sendEtherState === 'init' &&
              <div>
                <p className="b coal mt0">How much do you wish to withdraw?</p>
                <p className="b coal mt0">
                  Current Balance:
                ...
                <div className="flex cxc mxb w5">
                  ...
                  <input
                    ...
                    step="any"
                    className="w3 pa2"
                    onChange={this.props.onEtherAmountChange}
                  />
                </div>
              </div>}
          ...
          {this.props.sendEtherState === 'init' &&
            <button
              onClick={this.props.handleEscrowRequest}
              className="mt5 grow"
            >
              Withdraw Ether
            </button>}
          {this.props.sendEtherState === 'sent' ||
            (this.props.sendEtherState === 'denied' &&
              <button onClick={this.props.close} className="mt5 grow">
                Back to Dashboard
              </button>)}
        </div>
      </div>
